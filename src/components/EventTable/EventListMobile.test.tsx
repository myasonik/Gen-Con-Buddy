...
async function renderList(
  ...
  opts: { typeDisplay?: TypeDisplay; showTypeIcon?: boolean; dayFormat?: DayFormat; timeZone?: TimeZone; timeFormat?: TimeFormat } = {},
...
test("hides day but shows start time when only startDateTime is visible", async () => {
  await renderList([makeEvent()], { day: false, startDateTime: true, endDateTime: false }, { timeFormat: "24h" });
...
test("hides all time info when day, startDateTime, and endDateTime are all false", async () => {
  await renderList([makeEvent()], { day: false, startDateTime: false, endDateTime: false }, { timeFormat: "24h" });
  expect(screen.queryByText(/Thu/)).not.toBeInTheDocument();
