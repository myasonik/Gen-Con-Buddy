...
test('eventType param uses EVENT_TYPES enum for label', () => {
  ...
  expect(result).toEqual([{ key: 'eventType', label: 'Type: RPG - Role Playing Game' }])
})
...
test('specialCategory uses CATEGORY enum for label', () => {
  ...
  expect(result).toEqual([{ key: 'specialCategory', label: 'Category: Premier Event' }])
})
...
test('plain string params show their value', () => {
  expect(getActiveFilters({ title: 'dragon' })).toEqual([{ key: 'title', label: 'Title: dragon' }])
  expect(getActiveFilters({ location: 'Hall A' })).toEqual([
    { key: 'location', label: 'Location: Hall A' },
  ])
  expect(getActiveFilters({ gmNames: 'Bob' })).toEqual([{ key: 'gmNames', label: 'GM: Bob' }])
})
...
export function getActiveFilters(params: SearchParams): ActiveFilter[] {
  ...
  if (params.eventType)
    add('eventType', `Type: ${EVENT_TYPES[params.eventType] ?? params.eventType}`)
  if (params.group) add('group', `Group: ${params.group}`)
  if (params.shortDescription) add('shortDescription', `Short desc: ${params.shortDescription}`)
  if (params.longDescription) add('longDescription', `Long desc: ${params.longDescription}`)
  ...
  if (params.startDateTime) add('startDateTime', fmtDateRange(params.startDateTime, 'Start: '))
  if (params.duration) add('duration', fmtRange(params.duration, 'Duration: ', 'hrs'))
  if (params.endDateTime) add('endDateTime', fmtDateRange(params.endDateTime, 'End: '))
  if (params.minPlayers) add('minPlayers', fmtRange(params.minPlayers, 'Min players: '))
  if (params.maxPlayers) add('maxPlayers', fmtRange(params.maxPlayers, 'Max players: '))
  ...
  if (params.roundNumber) add('roundNumber', fmtRange(params.roundNumber, 'Round: '))
  if (params.totalRounds) add('totalRounds', fmtRange(params.totalRounds, 'Total rounds: '))
  ...
  if (params.lastModified) add('lastModified', fmtDateRange(params.lastModified, 'Modified: '))
...
test('renders a chip for each active filter', () => {
  ...
  expect(screen.getByRole('button', { name: /Location: Hall A/ })).toBeInTheDocument()
})
  ...
  return (
    <div className={styles.bar} aria-label="Active filters">
      {filters.map(({ key, label }) => (
        <button key={key} type="button" className={styles.chip} onClick={() => onRemove(key)}>
...
test('days active filter chip appears when days param is in URL', async () => {
  ...
  expect(screen.getByRole('button', { name: /Days: Fri, Sat/ })).toBeInTheDocument()
})
```
